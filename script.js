function obterUrlImagem(tema, largura = 800, altura = 500) {
    const dicionario = {
    ...
    const termoBaixo = tema.toLowerCase().trim();
    const tags = dicionario[termoBaixo] || termoBaixo;
    const randomId = Math.floor(Math.random() * 1000);

    return `https://loremflickr.com/${largura}/${altura}/${tags}?random=${randomId}`;
}

function gerarSiteCompleto(temaDigitado) {
    const urlPrincipal = obterUrlImagem(temaDigitado, 800, 500);
    const urlCliente = obterUrlImagem('cliente', 150, 150);
        ...
        <section class="hero-section" style="background-color: #fceade; padding: 40px 20px; text-align: center;">
            <div class="container" style="max-width: 600px; margin: 0 auto;">
                <div class="image-wrapper" style="margin-bottom: 20px;">
                    <img src="${urlPrincipal}" alt="${temaDigitado}" style="width: 100%; max-width: 400px; border-radius: 8px; display: block; margin: 0 auto;">
                </div>
                <h1 style="font-size: 2.5rem; margin-bottom: 10px; text-transform: capitalize;">${temaDigitado}</h1>
        ...
        <section class="depoimento-section" style="padding: 40px 20px; background-color: #ffffff; text-align: center;">
            <div class="container" style="max-width: 600px; margin: 0 auto;">
                ...
                <div class="card-depoimento" style="display: flex; flex-direction: column; align-items: center; gap: 15px;">
                    <img src="${urlCliente}" alt="Cliente" style="width: 80px; height: 80px; border-radius: 50%; object-fit: cover;">
